...
#  This is for performance considerations since pasting entire Worksheets in one shot is way faster than filing cells one by one using CDO.
...
$ThrottlingBatchSize = 95                       # Max number of unthrottled consecutive WebRequest queries to Okta platform
...
$FilteredAssignments | sort -Property ActionTimeStamp -Descending | export-csv "$($script:MyDocsFolder)\$($script:ExecutionTimeStamp)_AllAppAssignments_in_last_$($LookBackPeriod)_days.csv" -NoTypeInformation
$FilteredAssignments | sort -Property ActionTimeStamp -Descending | export-csv "$($script:MyDocsFolder)\$($script:ExecutionTimeStamp)_AllAppAssignments_in_last_$($LookBackPeriod)_days_UTF8.csv" -NoTypeInformation -Encoding UTF8
...
$Workbook.SaveAs("$($script:MyDocsFolder)\$($script:ExecutionTimeStamp)_AppPermissionsEvents_in_last_$($LookBackPeriod)_days.xlsx")
$Workbook.Close()
$objExcel.Quit()
